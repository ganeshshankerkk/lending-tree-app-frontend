...
function AllPendingTickets(){
            ...
            .then(data => {
                const ticketRecordsDetail = data.map(ticketDetails => {
                    return {
                        ...
                        remark: ticketDetails.remarks,
                    }
                })
                setPendingTickets(ticketRecordsDetail);
    ...
    function filterTickets(ticketId) {
        let filterTicketData = pendingTickets.filter(function (ticketRecords) {
    ...
    return(
        <Fragment>
     <h1 className="bg-dark mt-4 text-light pt-3 pb-3 display-6 text-center">Pending Tickets</h1>
                                ...
                                <div className="btn-toolbar" role="toolbar">
                                    <div className="btn-group me-2 mb-2" role="group">
                                        <button className="btn btn-info" onClick={() => filterTickets(ticket.ticketid)}>
                                            <Link className="text-decoration-none text-dark" to={`/pending-tickets/${ticket.ticketid}`}><BsCardList /> Details</Link>
                                        </button>
                                    </div>
                                </div>
                            </div>
...
export default AllPendingTickets;