...
function FilteredPersonVerificationRequest() {
    ...
    useEffect(() => {
        ...
        let userid = filteredLoanDataById.map((user) => {
            return user.id;
        })
    ...
    return (
        <Fragment>
            <h1 className="bg-dark mt-4 text-light pt-3 pb-3 display-5 text-center">Person & Loan Details</h1>
            {!isRejectingLoan && <div>
                    ...
                    {filteredLoanDataById.map((filteredUserData) => (
                        <table className="table table-striped " key={filteredUserData.loanId}>
                            <tbody>
                                ...
                                <tr>
                                    <th>First Name</th>
                                    <td>{individualUserData.firstName}</td>
                                </tr>
                                <tr>
                                    <th>Last Name</th>
                                    <td>{individualUserData.lastName}</td>
                                </tr>
                                <tr>
                                    <th>Email</th>
                                    <td>{individualUserData.email}</td>
                                </tr>
                                <tr>
                                    <th>Phone Number</th>
                                    <td>{individualUserData.contactNumber}</td>
                                </tr>
                                <tr>
                                    <th>Date Of Birth</th>
                                    <td>{individualUserData.dateOfBirth}</td>
                                </tr>
                                <tr>
                                    <th>Gender</th>
                                    <td>{individualUserData.gender}</td>
                                </tr>
                                <tr>
                                    <th>Amount</th>
                ...
                <div className="btn-toolbar text-center justify-content-center mb-5" role="toolbar">
                    <div className="btn-group me-2 mb-2" role="group">
                        <button className="btn btn-dark"><Link className="text-decoration-none text-white" to="/verification/pending"><ImArrowLeft2 /> Back</Link></button> &nbsp;
                    </div>
                    <div className="btn-group me-2 mb-2" role="group">
...
export default FilteredPersonVerificationRequest;