...
function ViewAllRejectedLoans() {
    ...
    useEffect(() => {
        ...
        fetch('http://localhost:8897/rejected-loans/all/')
            .then(response => {
    ...
    return (
        <Fragment>
            ...
            {/* <div className="col-lg-2 float-end">
                <select className="form-select form-select mb-3" onChange={dropdownChangeHandler}>
                    <option value="">Dropped By</option>
                    <option value="assignedToApprovalAgency">Approval Agency</option>
                    <option value="assignedToPickUpAgent">Pick Up Agent</option>
                    <option value="assignedToVerificationAgent">Person Verification Agent</option>
                    <option value="assignedToLegalDepartment">Legal Verification Agent</option>
                </select>
            </div> */}
                                ...
                                <div className="btn-toolbar" role="toolbar">
                                    <div className="btn-group me-2 mb-2" role="group">
                                        <button className="btn btn-info" >
                                            <Link className="text-decoration-none text-dark" onClick={() => filterRejectedLoan(loan.loanId)} to={`/loans/rejected/${loan.loanId}`}><BsCardList /> Details</Link>
                                        </button>
                                    </div>
                                </div>
                            </div>
...
export default ViewAllRejectedLoans;