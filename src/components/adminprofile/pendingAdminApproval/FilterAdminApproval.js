...
function FilterAdminApproval() {
    ...
    const [isAssigningToPickUpAgent, setIsAssigningToPickUpAgent] = useState(false);
    const required = <span className="text-danger">*</span>
    const loadingData = useSelector(state => state.loadingState.isLoading);

    function choosePickUpAgent() {
        setIsAssigningToPickUpAgent(true)
    }
    ...
    function rejectLoan(rejectionRemarks) {
        ...
        fetch("http://localhost:8897/loanreject/id=" + loanId + "/remarks=" + remarks + "/", {
            method: "PATCH",
    ...
    return (
        <Fragment>
            <h1 className="bg-dark mt-4 text-light pt-3 pb-3 display-5 text-center">{isAssigningToPickUpAgent ? 'Available Pick Up Agents' : 'Pending Loan Requests'}</h1>
            {loadingData && <div className="text-center mt-3">
                <div className="spinner-border" role="status">
                </div>
            </div>}
            {!isRejectingLoan && !isAssigningToPickUpAgent && <div>
                ...
                <div className="btn-toolbar text-center justify-content-center" role="toolbar">
                    <div className="btn-group me-2 mb-2" role="group">
                        <button className="btn btn-dark"><Link className="text-decoration-none text-white" to="/pending-loan/"><ImArrowLeft2 /> Go Back</Link></button> &nbsp;
                    </div>
                    <div className="btn-group me-2 mb-2" role="group">
                        <button className="btn btn-success" onClick={choosePickUpAgent}>Assign to Pick Up Employee <IoIosPersonAdd /></button>&nbsp;
                    </div>
                    <div className="btn-group me-2 mb-2" role="group">
            ...
            {!isRejectingLoan && isAssigningToPickUpAgent && <AvailablePickUpAgents />}
        </Fragment>
    )
}
export default FilterAdminApproval;