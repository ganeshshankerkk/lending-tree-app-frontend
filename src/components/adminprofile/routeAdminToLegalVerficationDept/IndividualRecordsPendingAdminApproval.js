...
function IndividualRecordsPendingAdminApproval() {
    ...
    const [isAssigningToLegalDeptAgent, setIsAssigningToLegalDeptAgent] = useState(false);
    const required = <span className="text-danger">*</span>
    const loadingData = useSelector(state => state.loadingState.isLoading);

    function chooseVerificationDeptAgent() {
        setIsAssigningToLegalDeptAgent(true)
    }
    ...
    return (
        <Fragment>
            <h1 className="bg-dark mt-4 text-light pt-3 pb-3 display-5 text-center">{isAssigningToLegalDeptAgent ? 'Available Legal Verification Department Agents' : 'Pending Loan Requests'}</h1>
            {loadingData && <div className="text-center mt-3">
                <div className="spinner-border" role="status">
                </div>
            </div>}
            {!isRejectingLoan && !isAssigningToLegalDeptAgent && <div>
                ...
                <div className="btn-toolbar text-center justify-content-center" role="toolbar">
                    <div className="btn-group me-2 mb-2" role="group">
                        <button className="btn btn-dark"><Link className="text-decoration-none text-white" to="/legal-verification"><ImArrowLeft2 /> Back</Link></button> &nbsp;
                    </div>
                    <div className="btn-group me-2 mb-2" role="group">
                        <button className="btn btn-success" onClick={chooseVerificationDeptAgent}>Map to Legal Dept Employee <IoIosPersonAdd /></button>&nbsp;
                    </div>
                    <div className="btn-group me-2 mb-2" role="group">
           ...
           {!isRejectingLoan && isAssigningToLegalDeptAgent && <AvailableLegalVerificationAgents/>} 
        </Fragment>
    )
}
export default IndividualRecordsPendingAdminApproval;